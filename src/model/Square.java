...
public class Square extends Shape {
	...
	public Square(double x1, double y1, double x2, double y2, Color color,
		...
		this.width = Math.min(Math.sqrt(Math.pow(x2 - x1, 2)),
	...
	protected java.awt.Shape getShape2D() {
		...
		java.awt.Shape rect = new Rectangle2D.Double(super.getX1(),
				super.getY1(), width, height);
	...
	protected Shape copyToHistory() {
		return new Square(getX1(), getY1(), getX2(), getY2(), getColor(),
	...
	protected void defineDimensions() {
		this.width = Math.min(Math.sqrt(Math.pow(getX2() - getX1(), 2)),
				Math.sqrt(Math.pow(getY2() - getY1(), 2)));
		this.height = width;
